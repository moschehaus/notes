...
\begin{proof}
	...
	\[
		\int_0^s <\partial_{tt}u(t),\psi>\dd{t} + \int_0^s \int_{\Omega}\tensorq{A}(t) \grad u(t) \vdot \grad \psi\dd{x} \dd{t} + \int_0^s \int_{\Omega}\qty(bu(t) + \vb{c} \vdot \grad u(t))\psi\dd{x} \dd{t} - \int_0^s \int_{\Omega}u(t) \vb{d}(t) \vdot \grad \psi\dd{x} \d{t} = 0,
	\]
...
\]
	...
	\]
	but since $\partial_t \psi(t) = - u(t),$ we can actually write (\textbf{notice we have kept $u(t)$ for Gronwall}:
	\[
		 \int_0^s<\partial_t u(t), u(t)> \dd{t}- \int_0^s \int_{\Omega}\tensorq{A}(t) \grad \partial_t \psi (t) \vdot \grad \psi\dd{x} \dd{t} - \int_0^s \int_{\Omega}\qty(-bu(t) + \vb{c} \vdot \grad \partial_t \psi(t))\psi\dd{x} \dd{t} + \int_0^s \int_{\Omega}\psi(t) \vb{d}(t) \vdot \grad \psi\dd{x} \d{t} = 0,
	\]
	and so
	\[
		\int_0^s \dv{t} \frac{1}{2}\qty(\norm{u(t)}_{\LpSet[2]{\Omega}}^{2}- \int_{\Omega}\tensorq{A}(t) \grad \psi(t) \vdot \grad \psi(t)\dd{x}) = - \int_0^s \int_{\Omega}\psi(t) \vb{d}(t) \vdot \grad \psi(t)\dd{x}\dd{t} + \int_0^s \int_{\Omega}\qty(-b u(t) + \vb{c} \vdot \grad  \psi(t))\psi\dd{x}\dd{t}.
	\]
	Integrating the LHS and realizing $u(0) = 0, \grad \psi (s) = 0,$yields
	\[
		\frac{1}{2}\qty(\norm{u(s)}_{\LpSet[2]{\Omega}}^{2} + \int_{\Omega}\tensorq{A}(0) \grad \psi(0) \vdot \grad\psi(0)\dd{x}) = - \int_0^s \int_{\Omega}\psi(t) \vb{d}(t) \vdot \grad \psi(t)\dd{x}\dd{t} + \int_0^s \int_{\Omega}\qty(-b u(t) - \vb{c} \vdot \grad  \psi(t))\psi\dd{x}\dd{t}.
	\]
	Using the ellipticity of $\tensorq{A}$ and the regularity of the data: \begin{align*}
		\int_{\Omega}\tensorq{A}(0) \grad \psi(0) \vdot \psi(0)\dd{x} &\geq C_1 \norm{\grad \psi(0)}_{\LpSet[2]{\Omega}} \Rightarrow - \int_{\Omega}\tensorq{A}(0) \grad \psi(0) \vdot \psi(0)\dd{x} \leq -C_1 \norm{\grad \psi(0)}_{\LpSet[2]{\Omega}} \leq -C_1 \norm{\psi(0)}_V, \\
		\int_{\Omega}\psi \vb{d} \vdot \grad \psi\dd{x} &\leq \norm{\vb{d}}_{\LinfSet{\Omega}}\norm{\psi}_{\LpSet[2]{\Omega}}\norm{\grad \psi}_{\LpSet[2]{\Omega}}\leq \norm{\vb{d}}_{\LinfSet{\Omega}}\norm{\psi}_{\WkpSet[1][2]{\Omega}}^{2}, \\
		\int_{\Omega}\qty(-bu + \vb{c} \vdot \grad \psi)\dd{x} &\leq \qty(\norm{b}_{\LinfSet{\Omega}} \norm{u}_{\LpSet[2]{\Omega}} + \norm{\vb{c}}_{\LinfSet{\Omega}}\norm{u}_{\WkpSet[1][2]{\Omega}}) \norm{\psi}_{\WkpSet[1][2]{\Omega}},
	\end{align*}
and so
\[
	\norm{u(s)}_{\LpSet[2]{\Omega}}^{2} + \norm{\psi(0)}_{\WkpSet[1][2]{\Omega}}^{2} \leq C\qty(\int_0^s \norm{\psi}_{\WkpSet[1][2]{\Omega}}^{2} + \norm{u}_{\LpSet[2]{\Omega}}^{2} + \norm{\psi(0)}_{\LpSet[2]{\Omega}}).
\]
If we now choose a test function such that $\chi(t) = \int_0^t u(\tau) \dd{\tau}, t \in (0,T],$ and repeat the whole procedure (again $\partial_t \chi(t) = u(t), \chi(0) = 0,$ we obtain the estimates
\[
	\norm{u(s)}_{\LpSet[2]{\Omega}}^{2} + \norm{\chi(s)}_{\WkpSet[1][2]{\Omega}}^{2} \leq C\qty(\int_0^s \norm{\chi(t) - \chi(s)}_{\WkpSet[1][2]{\Omega}}^{2} + \norm{u(t)}_{\LpSet[2]{\Omega}}^{2} + \norm{\chi(s)}_{\LpSet[2]{\Omega}}^{2}),
\]
and since $\norm{\chi(t) - \chi(s)}_{\WkpSet[1][2]{\Omega}}^{2} \leq 2 \qty(\norm{\chi(t)}_{\WkpSet[1][2]{\Omega}} + \norm{\chi(s)}_{\WkpSet[1][2]{\Omega}}^{2}),$ the above inequality in fact implies

\[
	\norm{u(s)}_{\LpSet[2]{\Omega}}^{2} + \qty(1-2 s C_1)\norm{\chi(s)}_{\WkpSet[1][2]{\Omega}}^{2} \leq C_1 \int_0^s \qty(\norm{\chi}_{\WkpSet[1][2]{\Omega}}^{2}+ \norm{u}_{\LpSet[2]{\Omega}}^{2} )\dd{t}.
\]
If we now choose $T_1 \leq T$ so small that $1-2T_1 C_1 \geq \frac{1}{2},$ and restrict ourselves on $s \in [0,T_1]$, combining the two estimates yields
\[
	\norm{u(s)}_{\LpSet[2]{\Omega}}^{2} + \norm{\chi(s)}_{\WkpSet[1][2]{\Omega}}^{2} \leq C \int_0^s\qty(\norm{u(t)}_{\LpSet[2]{\Omega}}^{2} + \norm{\chi(t)}_{\WkpSet[1][2]{\Omega}}^{2})\dd{t},
\]
...
\]
for $\xi \in \LpSet[1]{(0,T)}$ nonnegative.
