...
Since $\Omega_{j}^\delta$ is open there $\exists v_j \in \CinfSet{\Omega_j^{\delta}}$ such that
\[
	\norm{\tau_{\delta}u_j - v_j}_{\WkpSet{\Omega}} \leq \norm{\tau_{\delta}w_j - v_j}_{\WkpSet{\Omega_{j}^\delta}} < \frac{\varepsilon}{2(m+1)}.
\]
What is more, since $\overline{\Omega} \subset \Omega_j^{\delta},$ we see $v_j \in \CinfSet{\overline{\Omega}}$ in fact.
...
\[
	v = \sum_{j=0}^m v_j.
\]
Then $v \in \CinfSet{\overline{\Omega}}$ and it holds
\begin{align*}
...
\begin{remark}[What is $\, \text{C} \,^{\infty}_{\overline{\Omega}}\qty(\R^{d})$]
	...
	\]
In other literature, it is stated that also $\CinfSet{\overline{\Omega}}$ is dense in $\WkpSet{\Omega}$ if $\Omega \in \Ckl{0}{0}.$ This probably means
\[
	\CinfSet{\overline{\Omega}} \subset \, \text{C} \,^{\infty}_{\overline{\Omega}}\qty(\R^{d}).
\]
\end{remark}
