...
\end{theorem}
	...
	\]
	(that's just renaming $p$ with $r$ in Gagliardo-Nirenberg). Then it holds
	\[
	...
	Since functions from $\qty(EU)_{\delta}$ are finitely supported, there $\exists R>0$ \textit{s.t.}
	\[
		\forall u_{\delta} \in \qty(EU)_{\delta}: \supp u_{\delta} \subset \text{B}(0,R) \subset \R^d.
	\]
	(Also remember that $\Omega \subset \text{B}(0,R).$)
	Moreover, from the properties of mollification it follows: \footnote{We have something like $\CinfSet{K} \hookrightarrow \CkSet{1}{K}$ for $K$ compact.}
	\[
	...
	\]
	or in other words, all functions from $\qty(EU)_{\delta}$ are uniformly bounded and uniformly equicontinuous (that i s implied by uniform boundedness of the derivatives). Thus we can use Arzela-Ascoli theorem and state
	\[
		\CkSet{1}{\text{B}(0,R)} \hookrightarrow \hookrightarrow \CkSet{0}{\text{B}(0,R)}.
	\]
	This however implies $\CkSet{1}{\qty(\text{B}(0,R))}$ is relatively compact in $\CkSet{0}{\text{B}(0,R)}$ $\overbrace{\Rightarrow}^{\CkSet{1}{\text{B}(0,R)}\, \text{is complete} \,}$ totally bounded in $\CkSet{0}{\text{B}(0,R)}$ $\overbrace{\Rightarrow}^{\text{B}(0,R) \, \text{is compact} \,}$ totally bounded in $\LpSet[1]{\text{U}(0,R)}.$ The last implication follows from:\footnote{Take some sequence $\qty{f_k} \subset \CkSet{0}{\text{B}(0,R)}$ that is bounded, then
		\[
	...
	In total, $\qty(EU)_{\delta}$ is totally bounded in $\LpSet[1]{\text{U}(0,R)}.$
	Next, take an arbitrary $u \in U$ and compute (we are using the fact $Eu = u \, \text{\textit{a.e.}} \, \, \text{in} \, \Omega, \Omega \subset \text{U}(0,R).$)
	\begin{align*}
		\norm{u-\qty(Eu)_{\delta}}_{\LpSet[q]{\Omega}} &\leq \norm{\overbrace{Eu}^{\coloneq v}-(Eu)_{\delta}}_{\LpSet[q]{\text{U}(0,R)}} = \int_{\text{U}(0,R)}|v-v_{\delta}|\dd{x} = \int_{\R^d}|\int_{\R^d}v\qty(x+y)-v(x) \eta_{\delta}(y)\dd{y}|\dd{x}\leq \\
